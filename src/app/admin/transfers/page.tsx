...
export default function AdminTransfersPage() {
  ...
  const showActions = (t: Transfer) => {
    if (t.status === "pending" && canValidate) {
      return (
        <Button
          size="sm"
          variant="outline"
          className="text-xs border-[#C2E09D] text-[#2F3A2D] dark:text-[#E8EDE6] hover:bg-[#C2E09D]/20"
    ...
    if (t.status === "activated") {
      return <span className="text-xs text-[#64705E] dark:text-[#9BAA93]">Completado</span>
    }
    return null
  }
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#DDE7D3] dark:border-[#3A4536]/20">
                  <th className="text-left p-4 font-medium text-[#64705E] dark:text-[#9BAA93]">Cliente</th>
                  ...
                  <th className="text-left p-4 font-medium text-[#64705E] dark:text-[#9BAA93]">Referencia</th>
              ...
              <tbody>
                {loading ? (
                  <tr>
                    <td colSpan={7} className="p-8 text-center text-[#64705E] dark:text-[#9BAA93]">Cargando transferencias...</td>
                  </tr>
                ) : transfers.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="p-8 text-center text-[#64705E] dark:text-[#9BAA93]">No hay transferencias registradas</td>
                  </tr>
                ) : (
                  transfers.map((t) => {
                    const cfg = statusConfig[t.status] || { label: t.status, variant: "secondary" as const }
                    return (
                      <tr key={t.id} className="border-b border-[#DDE7D3] dark:border-[#3A4536]/10 hover:bg-[#F0F5EC] dark:hover:bg-[#2A3228] transition-colors">
                        <td className="p-4">
                          <p className="font-medium">{t.user.name || "—"}</p>
                          <p className="text-xs text-[#64705E] dark:text-[#9BAA93]">{t.user.email}</p>
                        </td>
                        <td className="p-4">{t.plan}</td>
                        <td className="p-4 font-medium">${t.amount.toFixed(2)}</td>
                        <td className="p-4 text-xs text-[#64705E] dark:text-[#9BAA93] font-mono">{t.referenceCode}</td>
                        <td className="p-4 text-xs text-[#64705E] dark:text-[#9BAA93]">{formatDate(t.createdAt)}</td>
                        <td className="p-4">
