    ...
    <div className="overflow-x-hidden">
      <div className="mb-8">
        ...
        <h1 className="text-2xl sm:text-3xl font-bold text-[#1A1A1A] mt-3">
          Gestión de <span style={{ color: "#88B078" }}>Telegram</span>
        </h1>
        <p className="text-sm text-[#666666] mt-1">Usuarios vinculados, notificaciones masivas y logs del bot</p>
      </div>
      ...
      <Card className="mb-6" style={{ backgroundColor: "white", borderColor: "#E8E8E8" }}>
        <CardContent className="p-5">
          <h2 className="text-base font-semibold mb-3 text-[#1A1A1A] flex items-center gap-2">
            <Send className="w-4 h-4 text-[#88B078]" />
          ...
          <div className="flex gap-2">
            ...
            />
            <Button onClick={handleBroadcast} disabled={sending || !broadcastText.trim()} style={{ backgroundColor: "#88B078", color: "#fff" }}>
              {sending ? "Enviando..." : "Enviar"}
            </Button>
          </div>
          <p className="text-xs text-[#666666] mt-2">{linkedUsers.length} usuarios recibirán el mensaje</p>
        </CardContent>
      </Card>
      ...
      <Card className="mb-6" style={{ backgroundColor: "white", borderColor: "#E8E8E8" }}>
        <CardContent className="p-4">
          <h2 className="text-sm font-semibold mb-3 text-[#1A1A1A]">API de Transmisión</h2>
          <p className="text-xs text-[#666666] mb-2">
            Usa este endpoint para enviar mensajes desde scripts externos:
          </p>
          <pre className="text-[10px] p-2 rounded" style={{ backgroundColor: "#F8F9FA", color: "#88B078" }}>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <Card style={{ backgroundColor: "white", borderColor: "#E8E8E8" }}>
          <CardContent className="p-5">
            <h2 className="text-base font-semibold mb-4 text-[#1A1A1A] flex items-center gap-2">
              <Users className="w-4 h-4 text-[#88B078]" />
              Usuarios Vinculados ({linkedUsers.length})
            </h2>
            {loading ? (
              <ListSkeleton rows={4} />
            ) : linkedUsers.length === 0 ? (
              <p className="text-[#666666] text-center py-4">Ningún usuario vinculado a Telegram</p>
            ) : (
              <div className="space-y-2 max-h-96 overflow-y-auto">
                {linkedUsers.map((user) => (
                  <div key={user.id} className="flex items-center justify-between p-3 rounded-lg" style={{ backgroundColor: "#F8F9FA" }}>
                    <div>
                      <p className="text-sm font-medium text-[#1A1A1A]">{user.name || user.email}</p>
                      <p className="text-xs text-[#666666]">{user.email} · {user.plan} · @{user.telegramId}</p>
                    </div>
                    <button
        ...
        <Card style={{ backgroundColor: "white", borderColor: "#E8E8E8" }}>
          <CardContent className="p-5">
            ...
            <pre className="p-3 rounded-lg text-[10px]" style={{ backgroundColor: "#F8F9FA", color: "#88B078" }}>
