  ...
  return (
    <div className="min-h-screen px-4 py-8">
      <div className="max-w-3xl mx-auto">
        ...
        {/* Current Plan */}
        <Card className={`p-6 mb-6 ${isPaid ? "ring-1 ring-[#E8D5C4]" : ""}`}>
          <CardContent className="p-0">
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className="text-sm text-[#8A7A6A]">Plan actual</p>
                <h2 className="font-serif text-2xl font-semibold text-[#3D3229]">{getPlanLabel(plan)}</h2>
              </div>
              <Badge className={isPaid ? "bg-[#E8D5C4] text-[#3D3229]" : "bg-[#F0F5EC] text-[#8A7A6A]"}>
                {isPaid ? "Activo" : "Gratuito"}
              </Badge>
            </div>

            {plan === "PRO_PLUS" && (
              <div className="mt-4 p-4 rounded-xl bg-[#E8D5C4]/10 border border-[#E8D5C4]">
                <p className="text-sm font-medium text-[#3D3229] mb-2">Funciones Pro+ exclusivas:</p>
                <ul className="space-y-1 text-xs text-[#8A7A6A]">
            ...
            {!isPaid && (
              <div className="mt-4 space-y-2">
                <p className="text-sm font-medium text-[#3D3229]">Actualizar a Premium — $4.99/mes</p>
                ...
                <Link href="/pricing" className="block text-center text-xs text-[#8A7A6A] hover:text-[#3D3229] mt-2">
        ...
        {usage && (
          <Card className="p-6 mb-6">
            <CardContent className="p-0">
              <h3 className="font-medium text-sm mb-4 flex items-center gap-2 text-[#3D3229]">
                <BarChart3 className="w-4 h-4 text-[#3D3229]" />
              ...
              <div className="space-y-3">
                {!usage.isUnlimited && (
                  <div>
                    <div className="flex justify-between text-sm mb-1">
                      <span className="text-[#8A7A6A]">Plan {getPlanLabel(usage.plan)}</span>
                      <span className="font-medium text-[#3D3229]">{usage.monthlyUsed} / {usage.monthlyLimit}</span>
                    </div>
                ...
                {usage.packTotal > 0 && (
                  <div>
                    <div className="flex justify-between text-sm mb-1">
                      <span className="text-[#8A7A6A]">Paquetes</span>
                      <span className="font-medium text-[#3D3229]">{usage.packTotal - usage.packRemaining} / {usage.packTotal}</span>
                    </div>
                    <div className="h-2 rounded-full bg-[#F0F5EC] overflow-hidden">
                      <div
                ...
                {usage.isUnlimited ? (
                  <p className="text-sm text-[#3D3229] flex items-center gap-1">
        ...
        {isPaid && (
          <Link href="/pricing?tab=packs">
            <Card className="transition-all duration-200 cursor-pointer hover:ring-1 hover:ring-[#E8D5C4]">
              <CardContent className="p-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <ShoppingBag className="w-5 h-5 text-[#3D3229]" />
                  <div>
                    <p className="text-sm font-medium text-[#3D3229]">Comprar más análisis</p>
                    <p className="text-xs text-[#8A7A6A]">Paquetes adicionales</p>
                  </div>
                </div>
                <CreditCard className="w-4 h-4 text-[#A89888]" />
              </CardContent>
            </Card>
          </Link>
        )}
        ...
        {payments.length > 0 && (
          <div>
            <h2 className="font-serif text-xl font-semibold mb-4 text-[#3D3229]">Historial de Pagos</h2>
                  ...
                  <CardContent className="p-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      {p.status === "completed" ? (
                        ...
                        <Clock className="w-5 h-5 text-[#8A7A6A]" />
                      ) : (
          ...
          <Card>
            <CardContent className="p-8 text-center">
              <div className="w-14 h-14 rounded-2xl bg-[#F0F5EC] flex items-center justify-center mx-auto mb-4">
                <CreditCard className="w-6 h-6 text-[#8A7A6A]" />
              </div>
              <p className="text-[#8A7A6A] mb-4">No tienes pagos registrados aún.</p>
              <Link href="/pricing">
