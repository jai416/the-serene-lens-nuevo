        ...
        <div className="flex flex-wrap gap-2">
          {observations.map((obs, i) => (
            <button
              ...
              className="px-3 py-1.5 bg-[#F8FAF5] dark:bg-gray-800 border border-[#DDE7D3] dark:border-gray-700 rounded-full text-sm hover:bg-[#C2E09D] hover:border-[#C2E09D] transition-colors"
            >
              {obs} 🔍
            </button>
          ))}
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl max-w-lg w-full max-h-[80vh] overflow-y-auto p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            {loading && (
              <div className="flex items-center justify-center py-12">
                <div className="animate-spin w-8 h-8 border-4 border-[#C2E09D] border-t-transparent rounded-full" />
            ...
            {explanation && (
              <div className="space-y-4">
                <div>
                  <h4 className="font-semibold text-[#2F3A2D] dark:text-white mb-1">📖 ¿Qué significa?</h4>
                  <p className="text-gray-600 dark:text-gray-300">{explanation.que_significa}</p>
                </div>

                {explanation.por_que_ocurre?.length > 0 && (
                  <div>
                    <h4 className="font-semibold text-[#2F3A2D] dark:text-white mb-1">🔬 ¿Por qué ocurre?</h4>
                ...
                {explanation.ingredientes_clave?.length > 0 && (
                  <div>
                    <h4 className="font-semibold text-[#2F3A2D] dark:text-white mb-2">🧴 Ingredientes clave</h4>
                    <div className="space-y-3">
                      {explanation.ingredientes_clave.map((ing, i) => (
                        <div key={i} className="p-3 bg-[#F8FAF5] dark:bg-gray-800 rounded-xl">
                          <p className="font-medium text-[#2F3A2D] dark:text-white">{ing.nombre}</p>
                          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{ing.para_que_sirve}</p>
                          <p className="text-sm text-[#64705E] dark:text-gray-400 mt-1">💡 <span className="italic">{ing.como_usarlo}</span></p>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {explanation.ajuste_rutina && (
                  <div className="p-3 bg-[#FFF6AD] dark:bg-yellow-900/30 rounded-xl">
                    <h4 className="font-semibold text-[#2F3A2D] dark:text-white mb-1">🔄 Ajusta tu rutina</h4>
                    <p className="text-gray-600 dark:text-gray-300">{explanation.ajuste_rutina}</p>
                  </div>
                )}

                {explanation.tiempo_mejora && (
                  <div className="p-3 bg-[#C2E09D]/20 dark:bg-green-900/20 rounded-xl">
                    <h4 className="font-semibold text-[#2F3A2D] dark:text-white mb-1">⏱️ Tiempo estimado</h4>
