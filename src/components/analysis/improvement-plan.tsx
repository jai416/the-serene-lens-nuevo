...
export function ImprovementPlan({ analysisId }: { analysisId: string }) {
  ...
  if (plan) {
    return (
      <div className="space-y-6">
        ...
        <div className="p-4 bg-[#88B078]/20 rounded-xl">
          <p className="font-semibold text-[#1A1A1A]">🎯 {plan.objetivo_principal}</p>
          <p className="text-gray-600 mt-1">{plan.resumen}</p>
        </div>
        {(plan.semanas || []).map((week) => (
          <div key={week.numero} className="border border-[#E8E8E8] rounded-xl p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">{week.enfoque}</p>
              </div>
            </div>
            <div className="space-y-3 ml-10">
              <div>
                <p className="text-sm font-medium text-[#666666] mb-1">🎯 Metas</p>
              ...
              {week.tips?.length > 0 && (
                <div>
                  <p className="text-sm font-medium text-[#666666] mb-1">💡 Tips</p>
                  <ul className="list-disc list-inside text-sm text-gray-600 space-y-0.5">
              ...
              {week.productos_sugeridos?.length > 0 && (
                <div>
                  <p className="text-sm font-medium text-[#666666] mb-1">🧴 Productos sugeridos</p>
                  <div className="flex flex-wrap gap-1">
                    {week.productos_sugeridos.map((p, i) => (
                      <span key={i} className="px-2 py-0.5 bg-[#F8F9FA] rounded-full text-xs">{p}</span>
                    ))}
                  </div>
                </div>
              )}
        ...
        <div className="p-4 bg-[#E2ECE0] rounded-xl">
          <h4 className="font-semibold text-[#1A1A1A] mb-1">📊 Seguimiento</h4>
          <p className="text-gray-600">{plan.seguimiento}</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="p-4 bg-[#F8F9FA] rounded-xl">
      ...
      <p className="text-sm text-gray-500 mb-4">Genera un plan personalizado con metas semanales basado en tu análisis de piel.</p>
