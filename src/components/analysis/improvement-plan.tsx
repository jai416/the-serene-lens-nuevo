...
export function ImprovementPlan({ analysisId }: { analysisId: string }) {
  ...
  if (plan) {
    return (
      <div className="space-y-6">
        ...
        <div className="p-4 bg-[#C2E09D]/20 dark:bg-green-900/20 rounded-xl">
          <p className="font-semibold text-[#2F3A2D] dark:text-white">🎯 {plan.objetivo_principal}</p>
          <p className="text-gray-600 dark:text-gray-300 mt-1">{plan.resumen}</p>
        </div>
        {plan.semanas.map((week) => (
          <div key={week.numero} className="border border-[#DDE7D3] dark:border-gray-700 rounded-xl p-4">
            <div className="flex items-center gap-2 mb-3">
              <span className="bg-[#C2E09D] text-[#2F3A2D] w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm">S{week.numero}</span>
              <div>
            ...
            <div className="space-y-3 ml-10">
              <div>
                <p className="text-sm font-medium text-[#64705E] dark:text-gray-400 mb-1">🎯 Metas</p>
              ...
              {week.tips?.length > 0 && (
                <div>
                  <p className="text-sm font-medium text-[#64705E] dark:text-gray-400 mb-1">💡 Tips</p>
                  <ul className="list-disc list-inside text-sm text-gray-600 dark:text-gray-300 space-y-0.5">
              ...
              {week.productos_sugeridos?.length > 0 && (
                <div>
                  <p className="text-sm font-medium text-[#64705E] dark:text-gray-400 mb-1">🧴 Productos sugeridos</p>
                  <div className="flex flex-wrap gap-1">
                    {week.productos_sugeridos.map((p, i) => (
                      <span key={i} className="px-2 py-0.5 bg-[#F8FAF5] dark:bg-gray-800 rounded-full text-xs">{p}</span>
                    ))}
                  </div>
                </div>
              )}
        ...
        <div className="p-4 bg-[#ECFFD3] dark:bg-green-900/20 rounded-xl">
          <h4 className="font-semibold text-[#2F3A2D] dark:text-white mb-1">📊 Seguimiento</h4>
  ...
  return (
    <div className="p-4 bg-[#F8FAF5] dark:bg-gray-800 rounded-xl">
      ...
      <button
        onClick={generate}
        disabled={loading}
        className="px-6 py-2.5 bg-[#C2E09D] hover:bg-[#B0CF8D] disabled:opacity-50 rounded-xl font-medium transition-colors flex items-center gap-2"
      >
        {loading ? (
          <>
